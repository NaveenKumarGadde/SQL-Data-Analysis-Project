...
SELECT * FROM youtube_statistics.`youtube statistics`;
 ...
 select subscribers , uploads, `video views` as video_views, highest_yearly_earnings from youtube_statistics.`youtube statistics`;

SELECT DISTINCT Country FROM youtube_statistics.`youtube statistics`;

-- 1 Finding the Top Continent in the Youtube Space, Subscribers, Uploads, earnings 
...
Select	Country,
 case
    ...
    subscribers , uploads, `video views` as video_views, highest_yearly_earnings from youtube_statistics.`youtube statistics`;
    
    
    -- Finding the top Continents 
    ...
    -- Using Common Table Expressions Concept
    
    with earlings_per as(
    Select	
     case
     when Country in ("Algeria","Angola","Benin","Botswana","Burkina Faso","Burundi","Cabo Verde","Cameroon","Central African Republic",
          "Chad","Comoros","Congo, Dem. Rep", "Congo, Rep.","Cote d'Ivoire","Djibouti","Egypt","Equatorial Guinea","Eritrea",
